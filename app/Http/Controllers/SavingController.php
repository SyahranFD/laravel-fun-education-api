...
class SavingController extends Controller
{
    ...
    public function showCurrent()
    {
        ...
        $pemasukanTerakhir = number_format($transaction->where('category', 'income')->sortByDesc('created_at')->first()['nominal'] ?? 0, 0, '.', '.');
        $pengeluaranTerakhir = number_format($transaction->where('category', 'outcome')->sortByDesc('created_at')->first()['nominal'] ?? 0, 0, '.', '.');
